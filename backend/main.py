...
# Load .env from repository root so agents can read API keys when uvicorn
# is started from the `backend` directory or repo root.
try:
...
# Add agents directory to path (keep for legacy relative imports)
...
# Import all agents. Prefer package-style imports (when running as module
# e.g. `python -m uvicorn backend.main:app`) but fall back to local imports
# when running from the `backend` folder directly.
try:
    from backend.agents.agent0_ocr import OCRAgent
    from backend.agents.agent1_scheduler import SchedulingAgent
    from backend.agents.agent2_forecast import ForecastAgent
    from backend.agents.agent3_compliance import ComplianceAgent
    from backend.agents.agent4_emergency import EmergencyAgent
    from backend.agents.agent_brightdata import BrightDataAgent
    from backend.agents.agent_memory import MemoryAgent
except Exception:
    # Fallback to local imports (existing behavior)
...
# Better error messages for JSON / validation problems
@app.exception_handler(RequestValidationError)
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    # Return a clearer message for client-side JSON or validation issues
    return JSONResponse(
        status_code=422,
        content={
            "detail": "Invalid request. Ensure you send valid JSON with Content-Type: application/json. For PowerShell use ConvertTo-Json to build payloads.",
            "errors": exc.errors()
        }
    )
...
async def generate_schedule(request: Request):
    ...
    This endpoint performs manual JSON parsing and validation to provide
    clearer error messages for malformed requests.
    """
    print("\n📅 [API] POST /api/generate-schedule called")

    # Parse JSON body with clear errors
    try:
    ...
    try:
        parsed = GenerateScheduleRequest.parse_obj(payload)
