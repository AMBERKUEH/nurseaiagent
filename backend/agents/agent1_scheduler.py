...
Generates and explains nurse schedules using Gemini API.
"""
...
def call_llm(prompt: str) -> str:
    ...
    try:
        ...
        raise ImportError(f"google-genai import failed: {e}. Run: pip install google-genai")
    ...
    response = client.models.generate_content(
        ...
        contents=f"You are an expert nurse scheduling AI for a hospital.\n\n{prompt}"
    )
...
No explanation, only JSON."""
        ...
        # Normalize schedule values: ensure each shift is a list of nurse names.
        def _to_list(value):
            ...
            if isinstance(value, str):
                ...
                if not s:
                    return []
                # Prefer comma-separated lists; otherwise split on whitespace
                if ',' in s:
                    parts = [p.strip() for p in s.split(',') if p.strip()]
                else:
                    parts = [p.strip() for p in re.split(r"\s+", s) if p.strip()]
                return parts
            # Fallback: wrap single non-empty values
            return [value] if value else []

        if isinstance(schedule, dict):
            for day, shifts in list(schedule.items()):
                if not isinstance(shifts, dict):
                    # If the LLM returned unexpected structure, skip normalization for this day
                    continue
                for shift, val in list(shifts.items()):
                    schedule[day][shift] = _to_list(val)
